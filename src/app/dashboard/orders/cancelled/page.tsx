...
import { Search, Filter, Eye, ArrowLeft, X, RotateCcw } from "lucide-react";
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            ...
            <p className="text-gray-600 font-medium">
              Orders that have been cancelled ({orders.length} orders)
            </p>
          </div>
        </div>
        ...
        {filteredOrders.length > 0 ? (
                    ...
                    {order.items.slice(0, 3).map((item: any, index: number) => (
                      <div key={index} className="relative w-8 h-8">
                        <Image
                          ...
                          alt={item.name}
                          fill
                          className="rounded-full border-2 border-white object-cover opacity-60"
                        />
                      </div>
                    ))}
                    {order.items.length > 3 && (
                      <div className="w-8 h-8 rounded-full bg-gray-100 border-2 border-white flex items-center justify-center text-xs font-bold text-gray-600 opacity-60">
                        +{order.items.length - 3}
                      </div>
                    )}
                  </div>
          ...
          <div className="modern-card p-12 text-center">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2">
              {orders.length === 0 ? "No Cancelled Orders" : "No Orders Found"}
            </h3>
